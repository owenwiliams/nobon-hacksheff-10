...
def create_new_entry(entry_in: EntryCreate, db: Session = Depends(get_db)):
...
def read_all_entries(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
...
def update_existing_entry(entry_id: int, entry_in: EntryUpdate, db: Session = Depends(get_db)):
